...
class MCTS():
    ...
    def getActionProb(self, game):
        ...
        if self.args.policy == "UCT":
            ...
            if 'Gaussian' in self.args.policy:
                ...
                for a in valids:
                    ...
                    if matched_child is None or not matched_child.Q:
                        # 无子节点或没有 Q 值
                        print(f"[跳过] 动作 {a} 没有匹配的子节点或 Q 为空")
                        ms[a] = root_node.V.get(a, 0.5)  # fallback 均值
                        ns[a] = 0
                        vas[a] = self.args.sigmaa_0  # 使用 prior 作为方差估计
                    else:
                        q_values = matched_child.Q
                        mean_Q = np.mean(q_values)
                        var_Q = np.var(q_values)

                        ms[a] = mean_Q if root_node.game.cur_player == 1 else 1 - mean_Q
                        ns[a] = matched_child.N
                        vas[a] = max(var_Q, EPS)  # 防止除以0或极小值

                    # 添加防御性编程，避免 sigmaa_0 过小
                    if self.args.sigmaa_0 < EPS:
                        raise ValueError(f"[错误] sigmaa_0={self.args.sigmaa_0} 太小，可能导致除以0")
                    print(1)
                    denom = (1 / self.args.sigmaa_0 + ns[a] / vas[a])
                    if denom < EPS:
                        print(f"[警告] 动作 {a} 的 pv 分母太小 ({denom})，强制设置为 EPS")
                        denom = EPS
                    pv[a] = 1 / denom

                    # 使用 prior mean V[a] 的默认值
                    v_a = root_node.V.get(a, 0.5)

                    pm[a] = pv[a] * (v_a / self.args.sigmaa_0 + ns[a] * ms[a] / vas[a])
                    ...
                    # 调试输出
                    print(f"[Gaussian动作 {a}] ms={ms[a]:.4f}, ns={ns[a]}, vas={vas[a]:.6f}, pv={pv[a]:.6f}, pm={pm[a]:.6f}")
        ...
        counts = np.array([x**0.5 for x in counts], dtype=np.float64)
        sum_counts = np.sum(counts)
        probs = (counts / sum_counts).tolist()
